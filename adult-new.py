...
file_url = 'https://raw.githubusercontent.com/ShadowWhisperer/BlockLists/master/RAW/Adult'
...
def insert_names_into_table(names, db_params):
    ...
    print(f"Inserting names into the table 'blocklist' with category 'porn' and source 'github(ShadowWhisperer)'...")
    for value in values:
        name = value[0]
        if not is_name_present(cursor, name):
            print(f"Inserting {name}")
            cursor.execute('INSERT INTO blocklist (domain, category, source) VALUES (%s, %s, %s)', (name, 'porn', 'github(ShadowWhisperer)'))
            conn.commit()
        else:
