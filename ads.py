...
file_url = 'https://raw.githubusercontent.com/ShadowWhisperer/BlockLists/master/RAW/Ads'
...
def insert_names_into_table(names, db_params):
    ...
    for value in values:
        ...
        if not is_name_present(cursor, name):
            ...
            cursor.execute('INSERT INTO blocklist (domain, category, source) VALUES (%s, %s, %s)', (name, 'ads', 'github(ShadowWhisperer)'))
            conn.commit()
        else:
